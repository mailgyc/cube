...
struct Block {
	unsigned char type;             // one of the above
	char floor, ceil;           	// height, in cubes
	unsigned char wtex, ftex, ctex; // wall/floor/ceil texture ids
	unsigned char r, g, b;       	// light value at upper left vertex
	unsigned char vdelta;        	// vertex delta, used for heightfield cubes
	char defer;  				 	// used in mipmapping, when true this cube is not a perfect mip
	char occluded;               	// true when occluded
	unsigned char utex;          	// upper wall tex id
	unsigned char tag;           	// used by triggers
};
...
enum {
	CS_ALIVE = 0, CS_DEAD, CS_LAGGED, CS_EDITING
};
