		...
		if (e.type == MAPMODEL) {
			...
			if (e.type != CARROT) {
				...
				case 0:
					...
					renderent(e, "carrot",
							(float) (1
									+ sin(lastmillis / 100.0 + e.x + e.y) / 20),
							lastmillis / (e.attr2 ? 1.0f : 10.0f));
					break;

				case 4:
					renderent(e, "switch2", 3, (float) e.attr3 * 90,
							(!e.spawned && !triggertime) ? 1 : 0,
							(e.spawned || !triggertime) ? 1 : 2, triggertime,
							1050.0f);
					break;
				case 5:
					renderent(e, "switch1", -0.15f, (float) e.attr3 * 90,
							(!e.spawned && !triggertime) ? 30 : 0,
							(e.spawned || !triggertime) ? 1 : 30, triggertime,
							35.0f);
					break;
				};
		};
...
struct itemstat {
	int add, max, sound;
		...
		25, S_ITEMAMMO, 25, 100, S_ITEMHEALTH, 50, 200, S_ITEMHEALTH, 100, 100,
		S_ITEMARMOUR, 150, 150, S_ITEMARMOUR, 20000, 30000, S_ITEMPUP, };
...
void checkitems() {
	...
	for(int i = 0; i < entityList.size(); ++i) {
		...
		if (OUTBORD(e.x, e.y))
			continue;
		Vec3 v = { (float)e.x, (float)e.y, (float)(S(e.x, e.y)->floor + player1->eyeheight) };
