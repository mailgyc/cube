...
monstertypes[NUMMONSTERTYPES] = { { GUN_FIREBALL, 15, 100, 3, 0, 100, 800, 1,
		10, 10, S_PAINO, S_DIE1, "an ogre", "monster/ogro" },
		{ GUN_CG, 18, 70, 2, 70, 10, 400, 2, 8, 9, S_PAINR, S_DEATHR, "a rhino",
				"monster/rhino" }, { GUN_SG, 14, 120, 1, 100, 300, 400, 4, 14,
				14, S_PAINE, S_DEATHE, "ratamahatta", "monster/rat" }, {
				GUN_RIFLE, 15, 200, 1, 80, 300, 300, 4, 18, 18, S_PAINS,
				S_DEATHS, "a slith", "monster/slith" },
		{ GUN_RL, 13, 500, 1, 0, 100, 200, 6, 24, 24, S_PAINB, S_DEATHB,
				"bauul", "monster/bauul" }, { GUN_BITE, 22, 50, 3, 0, 100, 400,
				1, 12, 15, S_PAINP, S_PIGGR2, "a hellpig", "monster/hellpig" },
		{ GUN_ICEBALL, 12, 250, 1, 0, 10, 400, 6, 18, 18, S_PAINH, S_DEATHH,
				"a knight", "monster/knight" }, { GUN_SLIMEBALL, 15, 100, 1, 0,
...
void monsteraction(Sprite *m) // main AI thinking routine, called every frame for every monster
	...
	if (m->blocked)                      // special case: if we run into scenery
	{
		...
		if (!rnd(20000 / monstertypes[m->mtype].speed)) // try to jump over obstackle (rare)
			...
			m->jumpnext = true;
				...
				&& (m->monsterstate != M_HOME || !rnd(5))) // search for a way around (common)
				{
	...
	float enemyyaw = -(float) atan2(m->enemy->o.x - m->o.x,
			m->enemy->o.y - m->o.y) / PI * 180 + 180;
	...
	case M_SLEEP:  // state classic sp monster start in, wait for visual contact
	{
		...
		|| (disttoenemy < 16 && angle < 135) || (disttoenemy < 32 && angle < 90)
...
void monsterrender() {
	loopv(monsters)
		renderclient(monsters[i], false,
				monstertypes[monsters[i]->mtype].mdlname,
				monsters[i]->mtype == 5,
